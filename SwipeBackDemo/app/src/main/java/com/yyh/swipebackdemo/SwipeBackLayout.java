...
public class SwipeBackLayout extends FrameLayout {
    ...
    private WeakReference<SwipeBackCallBack> mWeakReference;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (mContentView.getScrollX() <= -viewWidth / 2) {
                    isFinish = true;
                    scrollRight();//滚动出界面(finish)
                } else {
                    scrollOrigin();//滚动到起始位置
                    isFinish = false;
                }
                break;
        }
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            if (mScroller.isFinished() && isFinish) {
                ...
                if (mWeakReference != null) {
                    mWeakReference.get().onSwipeBackFinishListener();
                }
            }
        }
    }
    ...
    public interface SwipeBackCallBack {
        void onSwipeBackFinishListener();
    }

    public SwipeBackCallBack setCallBack(SwipeBackCallBack callBack) {
